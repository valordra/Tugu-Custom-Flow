...
import filesaverImport from '@salesforce/resourceUrl/filesaverJS';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';

export default class ExportAsDocx extends LightningElement {
    @api recordId;
    ...
    async loadScripts(){
        try {
            ...
            console.log('Loaded docx.');
            this.isScriptLoaded = true;
            // console.log(FileSaver);
        } catch (error) {
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Error loading external JS',
    ...
    generateDocument(){
        ...
        docx.Packer.toBlob(document).then((blob) => { 
            ...
            let editBlob = blob.slice(0, blob.size, "application/zip");

            console.log(editBlob);
            saveAs(editBlob, "example.docx");
        });
    }
} 